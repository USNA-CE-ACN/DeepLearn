...
def get_model(input_shape = (18,1), num_classes = 9, learning_rate = 1e-4, l2_rate = 1e-4, kernel_size = 3, strides = 1):

  model = models.Sequential()

  model.add(Conv1D(
      64, kernel_size = kernel_size, #kernel size = 3
			strides = strides,
			#activation = "relu",
			padding = "valid",
...
def main():
      ...
      test_full = np.load("../18vec_dataset/18vec_testset_round0.npy")
      ...
      for i in range(25):
          ...
          for x in range(0,101,10):
              ...
              train_full = np.load("../18vec_dataset/18vec_trainset_round0_rot"+str(x)+".npy")
          ...
          np.save("acc_total_18vec_CNNraw",acc_total)
          np.save("time_total_18vec_CNNraw",time_total)
