...
def main():
    ...
    for i in range(10):
        ...
        X_test, Y_test = test[:,:-1], test[:,-1]
        np.save('trainset_orig_70_7feb_round' + str(i), train_orig) #save original train dataset
        np.save('testset_orig_30_7feb_round' + str(i), test) #save the original test dataset
        
        model = traintest_model(train_orig,test,'0x',epochs=100)
        ...
        for x in range(10,101,10):
            ...
            np.save("7feb_1deg_acc_round"+str(i), acc_round)

            #save the model
            model.save('dnn_1deg_7feb_round' + str(i) + 'rotnum_' + str(x))

        acc_total.append(acc_round) #add to total accuracy

        #save final accuracy array at the end of every round
        np.save("1deg_acc_7feb", acc_total) #saving accuracy to 18 january
        np.savetxt("textfileacc",acc_total)

        f.write("round " + str(i) + "time: " + str(time.time()-round_start) + "\n")
